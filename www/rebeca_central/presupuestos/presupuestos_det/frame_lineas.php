	...
	if ($retorno==0) {
      ...
      $precio=str_replace(".","",($_POST["precio"]));
			$costototal=$cantidad*$precio;
                  ...
                  $sel_insert=" INSERT INTO presupuesto_det 	( 	pre_cod, 	pro_cod, 	prd_can, 	
                  prd_cos, 	prd_tot, 	fecha, 	hora, 	usuario, 	borrado 	) 	
                  ...
                  '$precio', 	'$costototal', 	'$fecha', 	'$hora', 	'$usuario', 	'0' 	)";
						      $rs_insert=mysql_query($sel_insert);
								...
								$consulta2="select sum(prd_tot) as total from presupuesto_det where pre_cod='$nro_factura' and borrado='0'";
								$rs_tabla2 = mysql_query($consulta2);
								$costotal=mysql_result($rs_tabla2,0,"total");	

							  $update_cab="update presupuesto_cab set pre_cos_tot='$costotal' where pre_cod='$nro_factura'";
								$rs_tabla3 = mysql_query($update_cab);
...
<?php 
$sel_lineas="SELECT * FROM presupuesto_det_view WHERE borrado=0 AND pre_cod='$nro_factura' 
ORDER BY prd_cod desc";
$costotal=0;
$rs_lineas=mysql_query($sel_lineas);
for ($i = 0; $i < mysql_num_rows($rs_lineas); $i++) {
	$numlinea=mysql_result($rs_lineas,$i,"prd_cod");
	...
	$pro_can=mysql_result($rs_lineas,$i,"prd_can");
	$pro_cos=mysql_result($rs_lineas,$i,"prd_cos");
	$pro_tot=mysql_result($rs_lineas,$i,"prd_tot");
