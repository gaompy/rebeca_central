...
$contenido = "select * from mesas where borrado=0 and mes_tip='1' order by mes_cod"; 
...
$query2="SELECT SUM(mes_est) AS suma FROM mesas WHERE borrado='0' and mes_cod > 0 and mes_tip='1'";
...
<table width="51%" height="128" border=1 align="center" cellpadding=10 cellspacing=20 class="fuente8">
  ...
  <tr>
    <?
    for ($i = 1; $i <= 7; $i++) {
        $sql1 = "SELECT MIN(mes_cod) AS minimo FROM mesas WHERE borrado='0' AND mes_cod > '$codigo' and mes_tip='1'"; 
        $query1 =mysql_query($sql1);
        $mes_cod=mysql_result($query1,0,"minimo");
     
        $sql = "select * from mesas where mes_cod='$mes_cod' and mes_cod > 0 and mes_tip='1'"; 
