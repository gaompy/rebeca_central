...
$contenido = "SELECT pro_cod,(SELECT pro_bar FROM productos WHERE pro_cod=compra_det.pro_cod)AS pro_bar,
(SELECT pro_des FROM productos WHERE pro_cod=compra_det.pro_cod)AS pro_des,
SUM(pro_can)AS pro_can, REPLACE(FORMAT(SUM(pro_tot),0),',','.')AS pro_tot
FROM compra_det WHERE borrado='0' AND fac_cod='$variable1'
GROUP BY pro_cod"; 
$queEmp = $contenido;
...
$txttit.= "<b></b>\n";
$txttit.= "<b></b>\n";
$txttit.= "<b>Detalle de Compras</b>\n";
$txttit.= "<b>Trans. Nro : $variable1</b>\n";
$txttit.= "<b>Factura Nro : $variable3</b>\n";
$txttit.= "<b>Proveedor : $variable2</b>\n";
$pdf->ezText($txttit, 12);
$pdf->ezTable($data, $titles, '', $options);
$pdf->ezText("<b>Item :</b> ".$ixx ."", 10);
$pdf->ezText("***************************************", 10);

$query1="SELECT tmo_cod,(SELECT tmo_des FROM tipo_moneda WHERE tmo_cod=compra_cab.tmo_cod)AS tmo_des
,SUM(cot_mon) AS cot_mon, (SELECT cot_ven FROM cotizaciones WHERE cot_cod=compra_cab.cot_cod) AS cot_ven 
FROM compra_cab  WHERE borrado=0 AND fac_cod='$variable1' AND tmo_cod >0
...
$query1="SELECT for_cod,(SELECT for_des FROM formas_pago WHERE for_cod=compra_cab.for_cod)AS for_des
...
$pdf->ezText("***************************************", 10);
$query1="SELECT mpg_cod,(SELECT mpg_des FROM medios_pago WHERE mpg_cod=compra_cab.mpg_cod)AS mpg_des
,SUM(fac_tot) AS fac_tot 
FROM compra_cab 
WHERE borrado='0' AND fac_cod='$variable1'
GROUP BY mpg_cod";
...
$pdf->ezText("***************************************", 10);
  /*
$query1="SELECT tie_cod,(SELECT tie_des FROM tipo_envio WHERE tie_cod=compra_cab.tie_cod)AS tie_des
,SUM(fac_tot) AS fac_tot 
FROM compra_cab 
WHERE borrado='0' AND fac_cod='$variable1'
GROUP BY tie_cod";
