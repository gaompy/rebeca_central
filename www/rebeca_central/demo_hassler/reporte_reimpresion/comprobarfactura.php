...
function pon_prefijo(cod_cli,cli_des,fecha,cli_ruc,nro_factura,total,tipo_des,importe) {
  ...
  parent.document.formulario.importe.value=importe;
...
function limpiar() {
	...
	parent.document.formulario.importe.value="";
...
<?
  ...
  if (mysql_num_rows($rs_tabla)>0) {
    ...
    $importe=number_format(mysql_result($rs_tabla,0,"fac_imp"), 0, ",", ".");
    ...
    $variable6=mysql_result($rs_tabla1,0,"tic_des");
		...
		pon_prefijo("<? echo $cli_cod ?>","<? echo $cli_des ?>","<? echo $fecha ?>","<? echo $cli_ruc ?>","<? echo $fac_cod ?>","<? echo $total ?>","<? echo $variable6 ?>","<? echo $importe ?>");
		</script>
		<? 
	} else { ?>
	<script>
