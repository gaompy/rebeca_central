...
if (!isset($nro_factura)) {
	...
	if ($retorno==0) {
      ...
      $preciototal=$cantidad*$precio;
                  ...
                  pro_cos,pro_ven ,	pro_tot_cos, pro_tot_ven	,fecha, 	hora, 	usuario, 	borrado 	) 	
                  VALUES 	( 	'$nro_factura', 	'$pro_cod', 	'$cantidad', 	
                  '$costo','$precio','$costototal', $preciototal	,'$fecha', 	'$hora', 	'$usuario', 	'0' 	)";
...
for ($i = 0; $i < mysql_num_rows($rs_lineas); $i++) {
  ...
  $pro_ven=mysql_result($rs_lineas,$i,"pro_ven");
	$pro_tot_cos=mysql_result($rs_lineas,$i,"pro_tot_cos");
  $pro_tot_ven=mysql_result($rs_lineas,$i,"pro_tot_ven");
  $costotal=$costotal+$pro_tot_cos;
  $pretotal=$pretotal+$pro_tot_ven;
...
//	if ($i % 2) { $fondolinea="itemParTabla"; } else { $fondolinea="itemImparTabla"; } ?>
		<!--	<tr class="<? echo $fondolinea?>"> -->
				...
				<td width="10%"><? echo number_format($pro_ven, 0, ",", ".");?></td>
...
<? } 
    $up_inv="update inventario_cab set pro_cos_tot='$costotal',pro_ven_tot='$pretotal'
    where inv_cod='$nro_factura'";
		$rs_inv = mysql_query($up_inv);
?>

</table>
<script>parent.document.formulario_lineas.referencia.focus();</script>
<script>parent.document.formulario_lineas.total_costo.value="<? echo number_format($costotal, 0, ",", ".");?>";</script>
<script>parent.document.formulario_lineas.total_venta.value="<? echo number_format($pretotal, 0, ",", ".");?>";</script>
