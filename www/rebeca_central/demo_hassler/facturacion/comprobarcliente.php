...
$consulta="SELECT * from clientes_view WHERE cli_cod='$cli_cod' and borrado='0'";
$rs_tabla = mysql_query($consulta);
	if (mysql_num_rows($rs_tabla)>0) {

		$cli_des=mysql_result($rs_tabla,0,"cli_des");	
		$cli_ruc=mysql_result($rs_tabla,0,"cli_ruc");
    ...
    $tic_por=mysql_result($rs_tabla,0,"tic_por");		
		...
		<script languaje="javascript">
	  	pon_prefijo("<? echo $cli_cod ?>","<? echo $cli_des ?>","<? echo $cli_ruc ?>","<? echo $tic_cod ?>","<? echo $tic_des ?>","<? echo $tic_por?>","<? echo $fac_cod?>");
