...
function excel() {
	...
	<body onLoad="inicio()">
		<div id="pagina">
           ...
           <?php
					  $query="SELECT * FROM usuarios where borrado=0 ORDER BY usuario ASC";
						$res=mysql_query($query);
						$contador=0;
					  ?>						
						
						<td width="15%">Usuarios</td>
							<td width="43%"><select id="variable5" name="variable5" class="comboMedio" onChange="buscar()">
							<option value="">Todos</option>
								<?php
								while ($contador < mysql_num_rows($res)) { ?>
								<option value="<?php echo mysql_result($res,$contador,"id")?>"><?php echo strtoupper(mysql_result($res,$contador,"usuario"))?></option>
								<?php $contador++;
								} ?>				
								</select>							</td>
				        </tr>
