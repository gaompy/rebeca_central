...
$variable_fac=$_POST["variable_fac"];
...
if ($variable_fac <> "") { $where.=" AND fac_num='$variable_fac'"; }

if ($variable5==1){
  if ($variable5 <> "") { $where.=" AND fac_num <>'0'"; }
    }else{
  if ($variable5 <> "") { $where.=" AND fac_num ='$variable5'"; }
  }


$where.=" ORDER BY fac_num desc";
$query_busqueda="SELECT count(*) as filas FROM factura_cab_view WHERE borrado=0 AND ".$where;
		...
		<div id="pagina">
			...
			<input type="hidden" name="numfilas" id="numfilas" value="<?php echo $filas; ?>">
				...
				if ($iniciopagina>$filas) { $iniciopagina=0; }
					if ($filas > 0) { ?>
						<?php $sel_resultado="SELECT *, (SELECT COUNT(*) FROM factura_imp WHERE borrado='0' AND fac_cod=factura_cab_view.fac_cod) AS cuenta_factura  
            FROM factura_cab_view WHERE borrado='0' and ".$where;
						   include("sql.php");
              ...
              <td width="12%"><div align="left"><?php echo strtoupper(mysql_result($res_resultado,$contador,"fecha"));?></div></td>
              
              <?php
           
              
               $cuenta_fac=mysql_result($res_resultado,$contador,"cuenta_factura");
                if ($cuenta_fac==0){
                $cuenta_fac="1";
                }
                
                $fac_num= mysql_result($res_resultado,$contador,"fac_num");
                if ($fac_num==0){
                 $cuenta_fac="S/F";
                }
              ?>
              <td width="25%"><div align="left"><?php echo $cuenta_fac;?></div></td>
              
              <td width="25%"><div align="left"><?php echo strtoupper(mysql_result($res_resultado,$contador,"fac_tot_1"));?></div></td>
						  <td width="5%"><div align="center"><a href="javascript:imprimir('<?php echo mysql_result($res_resultado,$contador,"fac_cod")?>','<?php echo mysql_result($res_resultado,$contador,"cli_des")?>')"><img src="../img/imprimir.png" width="16" height="16" border="0"  title="Imprimir"></a></div></td>
							</tr>
						<?php $contador++;
							}
						?>			
