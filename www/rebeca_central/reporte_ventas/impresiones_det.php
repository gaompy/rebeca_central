...
CASE 
WHEN
(SELECT pro_bol FROM productos WHERE pro_cod=factura_det.pro_cod)=0 THEN 'C/F'
WHEN
(SELECT pro_bol FROM productos WHERE pro_cod=factura_det.pro_cod)=1 THEN 'S/F'
END AS pro_bol,
...
SUM(pro_uti)AS pro_uti, REPLACE(FORMAT(SUM(pro_uti),0),',','.')AS pro_uti
...
$txttit.= "<b>Ticke Nro : $variable1</b>\n";
$txttit.= "<b>Cliente : $variable2</b>\n";


$query1="select * from factura_imp where borrado='0' and fac_cod='$variable1'";
$rs1=mysql_query($query1);

  for ($i = 0; $i < mysql_num_rows($rs1); $i++) {
       $fim_cod=mysql_result($rs1,$i,"fim_cod");
       $var1=mysql_result($rs1,$i,"fim_ini");
       $var2=mysql_result($rs1,$i,"fim_fin");
       $var3=mysql_result($rs1,$i,"fim_num");
       $nrofactura = str_pad($var1, 3, "0", STR_PAD_LEFT)."-".str_pad($var2, 3, "0", STR_PAD_LEFT)."-".str_pad($var3, 7, "0", STR_PAD_LEFT); 
       $txttit.= "Fact.Nro.: $nrofactura \n";
       //$pdf->ezText("<b></b>Fact.Nro.: $nrofactura ", 10);  
  } 
//$pdf->ezText("***************************************", 10);
