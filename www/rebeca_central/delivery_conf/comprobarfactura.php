...
function pon_prefijo(cod_cli,cli_des,fecha,cli_ruc,nro_factura,total,tipo_des,saldo,recibido) {
	...
	parent.document.formulario.total.value=total;	
	parent.document.formulario.diferencia.value=recibido;	
...
<?
	...
	$consulta="SELECT * FROM factura_cab WHERE fac_cod='$codfactura' AND borrado=0 and tie_cod='2'";
	...
	if (mysql_num_rows($rs_tabla)>0) {
		...
		$saldo=mysql_result($rs_tabla,0,"fac_sal");
		$fecha=mysql_result($rs_tabla,0,"fecha");
		$usuario=mysql_result($rs_tabla,0,"usuario");
		$recibido=mysql_result($rs_tabla,0,"fac_rec");			
    ...
    $variable6=mysql_result($rs_tabla1,0,"tic_des");
		...
		pon_prefijo("<? echo $cli_cod ?>","<? echo $cli_des ?>","<? echo $fecha ?>","<? echo $cli_ruc ?>","<? echo $fac_cod ?>","<? echo $total ?>","<? echo $variable6 ?>","<? echo $saldo ?>","<? echo $recibido ?>");
		</script>
		<? 
	} else { ?>
	<script>
