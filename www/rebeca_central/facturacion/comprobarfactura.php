...
function pon_prefijo(cod_cli,cli_des,cli_ruc,fecha,nro_factura,total,tip_cod,tipo_des,parametro) {
	parent.document.formulario.variable1.value=cod_cli;
	parent.document.formulario.variable2.value=cli_des;
	parent.document.formulario.variable3.value=cli_ruc;
	parent.document.formulario.variable4.value=tipo_des;	
...
<?
	...
	$consulta="SELECT * FROM factura_cab WHERE fac_cod='$codfactura'";
    ...
    $variable6=mysql_result($rs_tabla1,0,"tic_des");
		...
		pon_prefijo("<? echo $cli_cod ?>","<? echo $cli_des ?>","<? echo $cli_ruc ?>","<? echo $fecha ?>","<? echo $fac_cod ?>","<? echo $total ?>","<? echo $variable4 ?>","<? echo $variable6 ?>");
		</script>
		<? 
	} else { ?>
	<script>
